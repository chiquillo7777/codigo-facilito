...
public class SpringbootdemoApplication {
    ...
    public CommandLineRunner demo(ClienteRepository clienteRepository) {
        return args -> {
            ...
            log.info("Consultando clientes findAll()");
            clienteRepository.findAll().forEach(cliente -> log.info(cliente.toString()));

            log.info("");
            log.info("Consultando un cliente por ID 1");
            Cliente cliente = clienteRepository.findById(1L);
            log.info(cliente.toString());

            log.info("Consultar clientes con apellido chacon");
            clienteRepository.findByApellido("Chacon").forEach(chacon -> log.info(chacon.toString()));
        };
    }
}
